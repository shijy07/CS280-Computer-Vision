...
error=errs(ti(1),ri(1));
can=1;
for j=1:length(ti)
    if(errs(ti(j),ri(j))<error)
        can=j;
        error=errs(ti(j),ri(j));
    end
end

fprintf('\nReconstruction error = %f\n',errs(ti(can),ri(can)));

t2 = t{ti(can)}; R2 = R{ri(can)};
P2 = K2*[R2 t2];

% compute the 3D points with the final P2
[points,rec_err] = find_3d_points(matches,P1,P2); % <---------------------------------------------- You have already written this one!

%% -------- plot points and centers of cameras ----------------------------


plot_3d(points,K1,K2,P1,P2); % <-------------------------------------------------------------- You write this one!
